...
/**
 * Stripe webhook verifier. The {@code Stripe-Signature} header carries a timestamp and one or
 * more signatures: {@code t=<unix>,v1=<hex>,...}. The signed payload is
 * {@code <timestamp>.<rawBody>} (concatenated as bytes, never re-serialized), keyed by the
 * endpoint signing secret with HMAC-SHA256.
 *
 * <p>Only the {@code v1} scheme is trusted; the fake {@code v0} test scheme is ignored to prevent
 * downgrade attacks. The timestamp is checked against {@link VerificationContext#tolerance()} to
 * reject replays of old-but-valid deliveries.
 */
public final class StripeVerifier implements WebhookVerifier {
    ...
    private static final String V1_SCHEME = "v1";
    ...
    public VerificationResult verify(VerificationContext context) {
        ...
        for (String element : header.split(",")) {
            ...
            } else if (V1_SCHEME.equals(key)) {
                v1Signatures.add(value);
            }
            // Any other scheme (for example the fake v0) is ignored on purpose.
        }
